...
export default function DevisTuyauterie() {
  ...
  const handleGenerePDF = async () => {
    ...
    const ctx = await createSikaPDF(`DEVIS TUYAUTERIE - ${devisData.numero}`);
    ...
    const columns = ['Type', 'DN', 'Longueur (m)', 'Qté', 'PU (FCFA)', 'Montant (FCFA)'];
    ...
    await finalizeSikaPDF(ctx, `SIKA_Devis_Tuyauterie_${devisData.numero.replace(/\//g, '_')}.pdf`);
